...
import { CheckCircle2, Clock, AlertCircle, Flame, Star, Quote, Lock, ChevronRight, Timer, Menu, Check, XCircle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ appState, updatePrayerStatus, onOpenDrawer }) => {
  ...
  const handleStatusUpdate = (name: PrayerName, newStatus: PrayerStatus) => {
    ...
    // If clicking the current status, treat it as "unmarking" unless in strict mode
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        {PRAYER_NAMES.map((name) => {
          ...
          return (
            <div 
              key={name}
              className={`p-4 sm:p-5 rounded-[2rem] border transition-all duration-300 relative group flex flex-col justify-between ${cardStyle} ${shadowStyle} ${isLocked ? 'opacity-70' : ''}`}
            >
              ...
              <div className={`flex items-center justify-between gap-2 mt-auto ${isLocked ? 'pointer-events-none' : ''}`}>
                {/* Main Action: On Time */}
                <button 
                  type="button"
